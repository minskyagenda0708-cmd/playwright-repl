/**
 * CommandServer — HTTP server for the DevTools panel extension.
 *
 * Provides the POST /run endpoint so the panel can send commands
 * to the engine. The CDP relay is handled by Playwright's
 * ExtensionContextFactory + MCP Bridge extension.
 *
 * HTTP endpoints:
 *   POST /run   { "raw": "click e5" }  →  { "text": "Clicked", "isError": false }
 */
